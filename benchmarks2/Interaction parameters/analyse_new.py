        ...
        for compound in dat.dtype.names:
            ...
            for i,DeltaR in enumerate(data_compound[1]):
                if np.abs((data_compound[2][i]/t_max))>threshold:
                    print(f'vector:{DeltaR} and relative strength:{(data_compound[2][i]/t_max):.2f} and absolute strength:{data_compound[2][i]:.2f}')
            ...
            for i,DeltaR in enumerate(data_compound[3]):
                ...
                if np.abs(mat_avg/t_max)>threshold:
                    print(f'vector:{DeltaR} and relative strength:{mat_avg/t_max:.2f} and SU(6) breaking:{100*mat_error:.2g}% and absolute strength:{mat_avg:.2f}')
            print('='*100,'\n \n')
            #quit()
